...
cv::Mat Simulator::getCurrentLocationWithOffset() {
    locationLock.lock();
    cv::Mat locationCopy = Tcw.clone();
    locationLock.unlock();

    // Apply an offset of 0.3m to the x-coordinate
    locationCopy.at<double>(0, 3) += 0.3;
    return locationCopy;
}
...
Simulator::Simulator(std::string ORBSLAMConfigFile, std::string model_path, bool alignModelToTexture,
                     ...
                     std::string vocPath) : stopFlag(false), ready(false), saveMapSignal(false),
                                            ...
                                            viewportDesiredSize(1280, 480) { // Double the size to accommodate both cameras
...
void Simulator::simulatorRunThread() {
    ...
    pangolin::CreateWindowAndBind(windowName, viewportDesiredSize(0), viewportDesiredSize(1)); // Double the window size

    // we manually need to restore the properties of the context
    glEnable(GL_DEPTH_TEST);
    s_cam = pangolin::OpenGlRenderState(
            pangolin::ProjectionMatrix(640, 480, K(0, 0), K(1, 1), K(0, 2), K(1, 2), 0.1, 20),
    ...
    s_cam2 = pangolin::OpenGlRenderState(
            pangolin::ProjectionMatrix(640, 480, K(0, 0), K(1, 1), K(0, 2), K(1, 2), 0.1, 20),
            pangolin::ModelViewLookAt(0.1, -0.1, 0.3 + 0.3, 0, 0, 0, 0.0, -1.0,
    ...
    pangolin::RegisterKeyPressCallback('w', [&]() { applyForwardToModelCam(s_cam, movementFactor); applyForwardToModelCam(s_cam2, movementFactor); });
    pangolin::RegisterKeyPressCallback('a', [&]() { applyRightToModelCam(s_cam, movementFactor); applyRightToModelCam(s_cam2, movementFactor); });
    pangolin::RegisterKeyPressCallback('s', [&]() { applyForwardToModelCam(s_cam, -movementFactor); applyForwardToModelCam(s_cam2, -movementFactor); });
    pangolin::RegisterKeyPressCallback('d', [&]() { applyRightToModelCam(s_cam, -movementFactor); applyRightToModelCam(s_cam2, -movementFactor); });
    pangolin::RegisterKeyPressCallback('e', [&]() { applyYawRotationToModelCam(s_cam, 1); applyYawRotationToModelCam(s_cam2, 1); });
    pangolin::RegisterKeyPressCallback('q', [&]() { applyYawRotationToModelCam(s_cam, -1); applyYawRotationToModelCam(s_cam2, -1); });
    pangolin::RegisterKeyPressCallback('r', [&]() { applyUpModelCam(s_cam, -movementFactor); applyUpModelCam(s_cam2, -movementFactor); }); // ORBSLAM y axis is reversed
    pangolin::RegisterKeyPressCallback('f', [&]() { applyUpModelCam(s_cam, movementFactor); applyUpModelCam(s_cam2, movementFactor); });
    ...
    pangolin::Handler3D handler2(s_cam2);
    ...
    int commandsExecuted = 0; // Flag to ensure commands are only executed once
	...
	if (commandsExecuted <= 50) {
            // Execute commands here after the first frame is rendered
            //std::string moveCommand = "forward";
            int intervalUsleep = 1000; // 10ms sleep interval between movements
            double fps = 30.0; // Frames per second
            int totalCommandTimeInSeconds = 2; // Move for 2 seconds
            double moveValue = 0.01; // The distance to move the camera forward
	    double rotateValue = 1.0; // Rotate the camera clockwise by 10 degrees
            //command(moveCommand, intervalUsleep, fps, totalTimeInSeconds);
	    intervalOverCommand(Simulator::applyYawRotationToModelCam, rotateValue, intervalUsleep, fps, totalCommandTimeInSeconds);
	    intervalOverCommand(Simulator::applyForwardToModelCam, moveValue, intervalUsleep, fps, totalCommandTimeInSeconds);
            //std::string rotateCWCommand = "cw";
            //command(rotateCWCommand, intervalUsleep, fps, totalTimeInSeconds);

            commandsExecuted += 1; // Set the flag to true to prevent re-execution
        }

	if (commandsExecuted >= 50 && commandsExecuted < 100) {
            // Execute commands here after the first frame is rendered
            //std::string moveCommand = "forward";
            int intervalUsleep = 1000; // 10ms sleep interval between movements
            double fps = 30.0; // Frames per second
	...
	if (commandsExecuted >= 100 && commandsExecuted <= 130) {
            // Execute commands here after the first frame is rendered
            //std::string moveCommand = "forward";
            int intervalUsleep = 1000; // 10ms sleep interval between movements
            double fps = 30.0; // Frames per second
	...
	if (commandsExecuted >= 130 && commandsExecuted < 160) {
            // Execute commands here after the first frame is rendered
            //std::string moveCommand = "forward";
            int intervalUsleep = 1000; // 10ms sleep interval between movements
            double fps = 30.0; // Frames per second
